<template>
  <div :class="['gallery-item', data.checked ? 'gallery-item-active' : '']">
    ...
    <div class="gallery-item-name" :title="data.name">
      <span>{{ data.name || data.img_name }}</span>
    </div>
...
const actions = {
  down() {
    // 获取图片的URL
    const imageUrl = myData.value.url; // myData 是你的 computed 属性

  ...
  <com-dialog
    ...
    <div class="c-list">
      ...
      <div class="c-list-item" style="flex-wrap: wrap;" v-if="showAlbum">
        <div style="display: flex;width: 100%;align-items: center;">
          <span class="item-label">关联壁纸：</span>
        ...
        <p style="width: 100%;margin-top: 5px;color: var(--el-text-color-secondary);">选择壁纸后，点击加入壁纸按钮</p>
      </div>
    </div>
    <template #action>
...
const getAlbums = () => {
  album.find({}).then((res: PageResponse<AlbumInter, { id: string, count: number }>) => {
    albums.value = res.records.map(item => {
...
//     slient: true
//   }).then(res => {
//     ctx.$message({ message: '加入壁纸成功', duration: 1000, type: 'success' })
//   })
// }
// 保存名称
const saveName = () => {

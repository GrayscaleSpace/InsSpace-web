  ...
  <div class="gallery-container">
    <c-card :title="'壁纸'+'('+(list.total)+')'">
      ...
      <div class="gallery-filter">
        ...
        <div>
          <el-button type="primary" v-if="list.data.filter(el => el.checked).length" @click="copySelectLink">复制链接
          </el-button>
      ...
      <div class="gallery-list" v-loading="list.loading">
        <el-row v-if="list.data.length">
          <template v-for="(item, index) in list.data" :key="'gallery-item' + index">
            <el-col :xl="3" :lg="4" :md="7" :sm="11" :xs="24">
              <gallery-item
                  :data="item"
                  :key="item.id"
...
const listGet = () => {
  ...
  image.find({
    ...
    // 使用 useFormat 函数格式化记录的 createTime，但此处没有保存结果
...
function copySelectLink() {
  const coptxt = list.data.filter(el => el.checked).map(el => el.url).join('\n')
